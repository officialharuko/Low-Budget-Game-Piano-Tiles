...
c.addEventListener('touchstart', function(e) {
    ...
    for (var i = 0; i < numOfTiles; ++i) {
        if (eachState[i] && myTiles[i].live) {
            // Check if touch is within the tile
            if (
                x >= myTiles[i].x && x <= myTiles[i].x + myTiles[i].width &&
                y >= myTiles[i].y && y <= myTiles[i].y + myTiles[i].height
            ) {
                // Simulate correct key press if in scoring area
                if (myTiles[i].y < 470 && myTiles[i].y > 350) {
                    afterRight(i);
                }
            }
